...
//! Connects to Code Assist API (cloudcode-pa.googleapis.com) which is what Gemini CLI uses.
...
impl GeminiOAuthClient {
    ...
    /// Get project ID from cache or fetch via Code Assist API.
    async fn get_or_fetch_project_id(&self, access_token: &str) -> Result<String> {
        ...
        for env_var in ["GOOGLE_CLOUD_PROJECT", "GOOGLE_CLOUD_PROJECT_ID"] {
            if let Ok(project) = std::env::var(env_var) {
                if !project.is_empty() {
                    tracing::debug!("Using {}: {}", env_var, project);
                    return Ok(project);
                }
            }
        }

        // Use Code Assist API to load/discover project
        let load_body = json!({
            "metadata": {
    ...
    async fn onboard_user(&self, access_token: &str) -> Result<String> {
        ...
        if let Some(name) = data.get("name").and_then(|n| n.as_str()) {
            let is_done = data.get("done").and_then(|d| d.as_bool()).unwrap_or(false);
            if !is_done {
        ...
        Err(Error::Auth(
            "Could not provision project. Set GOOGLE_CLOUD_PROJECT or GOOGLE_CLOUD_PROJECT_ID."
                .to_string(),
        ))
    }
    ...
    async fn chat(&self, messages: &[Message], tools: &[ToolDefinition]) -> Result<LlmResponse> {
        ...
        loop {
            ...
            if response.status().is_success() {
                ...
                let inner_response = code_assist_response.get("response").ok_or_else(|| {
                    Error::Llm("Missing 'response' field in Code Assist response".to_string())
                })?;
            ...
            // Clippy fix: combined rate limit check
            let is_rate_limited =
                status.as_u16() == 429 || error_text.contains("RESOURCE_EXHAUSTED");
            if is_rate_limited && retry_count < max_retries {
                retry_count += 1;
            ...
            return Err(Error::Llm(format!(
                "Code Assist API error ({status}): {error_text}"
            )));
        }
    }
